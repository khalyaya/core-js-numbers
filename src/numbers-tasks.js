...
function getAverage(value1, value2) {
  const avg = Math.floor((value1 + value2) / 2);
  if (avg > Number.MAX_VALUE) {
...
function getFibonacciNumber(index) {
  ...
  do {
    ...
    const secondToLastNum = arr[arr.length - 2];
    const sum = lastNum + secondToLastNum;
    arr.push(sum);
...
function getSumOfDigits(num) {
  ...
  const arr = numToString.split('');
  const sum = arr.reduce((acc, current) => acc + parseInt(current, 10), 0);
  return sum;
}
...
function numberToStringInBase(number, base) {
  ...
  return parseInt(numberToString, 10).toString(base);
}
